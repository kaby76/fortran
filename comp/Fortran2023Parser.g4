...
allocation
    ...
    | (lower_bounds_expr COLON)? upper_bounds_expr (lbracket allocate_coarray_spec rbracket)?
    ;
...
computed_goto_stmt
    : GOTO LPAREN label_list RPAREN COMMA? scalar_int_expr
    ;
...
enum_def
    : enum_def_stmt enumerator_def_stmt+ end_enum_stmt
    ;

enum_def_stmt
    : ENUM COMMA BINDC (DOUBLECOLON enum_type_name)?
    ;
...
enumeration_type_def
    : enumeration_type_stmt enumeration_enumerator_stmt+ end_enumeration_type_stmt
    ;
...
format_stmt
    : label? FORMATIN format_specification RPAREN
    ;
...
int_literal_constant
    : DIGITSTRING (UNDERSCORE kind_param)?
    ;
...
language_binding_spec
    : BINDC (COMMA NAAM ASSIGN scalar_default_char_constant_expr)? RPAREN
    ;
...
type_attr_spec_list
    : type_param_attr_spec (COMMA type_param_attr_spec)*
    ;
